...
def _cosine_distance(a, b, data_is_normalized=False):
    ...
    if not data_is_normalized:
        a = np.asarray(a) / np.linalg.norm(a, axis=1, keepdims=True)  #1*n
        b = np.asarray(b) / np.linalg.norm(b, axis=1, keepdims=True)    #m*n
...
def _nn_euclidean_distance(x, y):
    ...
    m = np.maximum(0.0, torch.min(torch.from_numpy(distances), axis=0)[0].numpy())
    return m
...
class NearestNeighborDistanceMetric(object):
    ...
    def partial_fit(self, features, targets, active_targets):
        ...
        ----------
        features : ndarray
            An NxM matrix of N features of dimensionality M.,M维度，N个目标
        targets : ndarray
    ...
    def distance(self, features, targets):
        """Compute distance between features and targets.  #计算feature和targets间距离：feature：检测结果；sample【target】存档轨迹
        Parameters
        ----------
        features : ndarray
...
class NNTrackDistanceMetric(object):
    """
    A nearest neighbor distance metric that, for each target, returns
    the closest distance to any sample that has been observed so far.
    Parameters
    ...
    def __init__(self, metric, matching_threshold, budget=None):
        ...
        self.matching_threshold = matching_threshold
        self.budget = budget
        self.samples_visible = {}
        self.samples_infrared = {}


    def partial_fit(self, targets_visible, active_targets_visible, targets_infrared, active_targets_infrared):
        """Update the distance metric with new data.
        Parameters
        ----------
        features : ndarray
        ...
        """
        for feature, target in zip(features, targets):
            self.samples.setdefault(target, []).append(feature)  # 如果还没有某个轨迹，新建这个轨迹
            if self.budget is not None:
                self.samples[target] = self.samples[target][-self.budget:]  # 仅保留后面budget个特征
        self.samples_visible = {k: self.samples_visible[k] for k in active_targets_visible} #筛选有效轨迹
