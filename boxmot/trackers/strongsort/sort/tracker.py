...
class Tracker:
    ...
    def update(self, detections):
        ...
        """
        # Run matching cascade. 级联匹配，对可见光与红外轨迹各匹配一次
        matches, unmatched_tracks, unmatched_detections = self._match(detections)

        # Update track set. 更新轨迹集合
        for track_idx, detection_idx in matches:  # 更新匹配成功的轨迹集合：运行update
            self.tracks[track_idx].update(detections[detection_idx])
        for track_idx in unmatched_tracks:          # 没有成功匹配的轨迹：作为失踪轨迹处理
            self.tracks[track_idx].mark_missed()
        for detection_idx in unmatched_detections:  # 没有匹配成功的检测：作为初始化的轨迹
            self._initiate_track(detections[detection_idx])
        self.tracks = [t for t in self.tracks if not t.is_deleted()]  # 目前未被删除的轨迹，作为当前新的轨迹库

        # Update distance metric. 筛选出正在活跃的轨迹，滤波更新（区别于tentative）
